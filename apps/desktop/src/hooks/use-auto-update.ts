import type { UpdaterEventMap } from "@shared/host";
...
export function useAutoUpdate() {
  ...
  useEffect(() => {
    const updater = window.clawpiUpdater;
    if (!updater) return;
    ...
    disposers.push(
      updater.onEvent(
        "update:available",
        (data: UpdaterEventMap["update:available"]) => {
          setState((prev) => ({
            ...prev,
            phase: "available",
    ...
    disposers.push(
      updater.onEvent(
        "update:progress",
        (data: UpdaterEventMap["update:progress"]) => {
          setState((prev) => ({
            ...prev,
            phase: "downloading",
    ...
    disposers.push(
      updater.onEvent(
        "update:downloaded",
        (data: UpdaterEventMap["update:downloaded"]) => {
          setState((prev) => ({
            ...prev,
            phase: "ready",
            version: data.version,
