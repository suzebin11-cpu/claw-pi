...
export async function bootstrapWithLaunchd(
  ...
  for (let offset = 0; offset < WEB_PORT_ATTEMPTS; offset++) {
    ...
    try {
      ...
      if (
        code !== "EADDRINUSE" &&
        !(process.platform === "win32" && code === "EACCES")
      ) {
        throw err;
      }
      console.log(
...
async function removeDirectoryIfExists(targetPath: string): Promise<void> {
  ...
  assertSafeRmTarget(targetPath);
  await fs.rm(targetPath, { recursive: true, force: true });
}

async function copyDirectoryCloneAware(
  sourcePath: string,
  targetPath: string,
): Promise<void> {
  try {
...
function resolvePackagedOpenclawBinPath(openclawSidecarRoot: string): string {
  return path.join(
    openclawSidecarRoot,
    "bin",
 ...
 * Falls back to "Claw-Pi" if the plist cannot be parsed.
 */
function readBundleExecutableName(appContentsPath: string): string {
  const fallback = "Claw-Pi";
  try {
...
export async function ensureExternalNodeRunner(
  ...
  if (existsSync(stagingRoot)) {
    await removeDirectoryIfExists(stagingRoot).catch(() => {});
  }
  ...
  await copyDirectoryCloneAware(appBundlePath, stagingRoot);
  ...
  await removeDirectoryIfExists(runnerRoot).catch(() => {});
...
async function ensureExternalControllerSidecar(
  ...
  // If the process is killed mid-extraction, only the staging directory is
  // left behind and will be cleaned up on next startup (see above).
  await copyDirectoryCloneAware(srcControllerDir, stagingRoot);
  ...
  await removeDirectoryIfExists(controllerRoot).catch(() => {});
...
export async function resolveLaunchdPaths(
  ...
  if (isPackaged) {
    ...
    const nexuHome = path.join(os.homedir(), ".nexu");
    const nexuRuntimeRoot = path.join(nexuHome, "runtime");
    const version = appVersion ?? "unknown";
    const controllerRoot = path.join(runtimeDir, "controller");
    const controllerEntryPath = path.join(controllerRoot, "dist", "index.js");
    ...
    if (process.platform !== "darwin") {
      ...
      return {
        ...
        openclawBinPath: resolvePackagedOpenclawBinPath(openclawSidecarRoot),
    ...
    let effectiveControllerEntryPath = controllerEntryPath;
    let effectiveControllerRoot = controllerRoot;
    ...
    try {
      ...
      effectiveControllerEntryPath = result.entryPath;
      effectiveControllerRoot = result.controllerRoot;
    } catch (err) {
      console.error(
    ...
    // 3. OpenClaw sidecar is already extracted outside the app bundle.
    const openclawSidecarRoot = await ensurePackagedOpenclawSidecar(
      runtimeDir,
      nexuRuntimeRoot,
    );
    ...
    return {
      nodePath,
      controllerEntryPath: effectiveControllerEntryPath,
      openclawPath: path.join(
        openclawSidecarRoot,
        "node_modules",
        "openclaw",
      ...
      controllerCwd: effectiveControllerRoot,
      openclawCwd: openclawSidecarRoot,
      openclawBinPath: resolvePackagedOpenclawBinPath(openclawSidecarRoot),
      openclawExtensionsDir: path.join(
        openclawSidecarRoot,
