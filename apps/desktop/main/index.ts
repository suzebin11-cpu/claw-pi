...
const useExternalController =
  hasExplicitEnvValue(process.env.NEXU_CONTROLLER_URL) ||
  hasExplicitEnvValue(process.env.NEXU_CONTROLLER_BASE_URL) ||
  hasExplicitEnvValue(process.env.NEXU_API_URL) ||
  hasExplicitEnvValue(process.env.NEXU_API_BASE_URL);

const useDevEmbeddedWebServer =
  !app.isPackaged &&
  useExternalController &&
  (() => {
    try {
      return isLoopbackHostname(
        new URL(runtimeConfig.urls.controllerBase).hostname,
      );
    } catch {
      return false;
    }
...
// Keep the embedded workspace transparent only on macOS, where the window
// uses vibrancy. On Windows/Linux this transparency leaks through layout gaps
// (e.g. the workspace split gutter / rounded edge) and can show a pale strip
...
function readNativeCrashTestTitle(event: Sentry.Event): string | null {
  ...
  const electronContext = event.contexts?.electron as
    ...
    | undefined;
  const crashpadTitle = electronContext?.["crashpad.clawpi.crash_title"];
...
function readNativeCrashTestKind(event: Sentry.Event): string | null {
  ...
  const electronContext = event.contexts?.electron as
    | Record<string, unknown>
    | undefined;
  const crashpadKind = electronContext?.["crashpad.clawpi.crash_kind"];
...
let devEmbeddedWebServer: EmbeddedWebServer | null = null;
...
async function refreshProxyDiagnostics(): Promise<void> {
  ...
  const targets = [
    ...
    { label: "external", url: "https://claw-pi.cn" },
  ];
...
async function gracefulShutdown(reason: string): Promise<void> {
  ...
  try {
    ...
    if (devEmbeddedWebServer) {
      await devEmbeddedWebServer.close().catch(() => undefined);
      devEmbeddedWebServer = null;
    }
          ...
          void (async () => {
            ...
            if (result.status === "success" && result.outputPath) {
              ...
              await dialog.showMessageBox({
                ...
                title: "导出失败",
                message: "导出运行日志失败",
                detail: result.errorMessage ?? "未知错误",
              });
            }
          })();
        },
...
async function runDesktopColdStart(): Promise<void> {
  ...
  logColdStart(
    useExternalController
      ? `waiting for external controller readiness (${runtimeConfig.urls.controllerBase})`
      : "waiting for controller readiness",
  );
  await waitForControllerReadiness();

  if (useDevEmbeddedWebServer) {
    diagnosticsReporter?.markColdStartRunning("starting embedded web server");
    logColdStart("starting embedded web server");

    const controllerUrl = new URL(runtimeConfig.urls.controllerBase);
    const controllerPort = Number.parseInt(controllerUrl.port || "80", 10);
    const webRoot = resolve(electronRoot, "../web/dist");

    devEmbeddedWebServer = await startEmbeddedWebServer({
      port: runtimeConfig.ports.web,
      webRoot,
      controllerPort,
    });

    runtimeConfig.ports.web = devEmbeddedWebServer.port;
    runtimeConfig.urls.web = `http://127.0.0.1:${devEmbeddedWebServer.port}`;

    logColdStart(
      `embedded web server ready at ${runtimeConfig.urls.web} (root=${webRoot})`,
    );
  } else {
...
app.on("web-contents-created", (_event, contents) => {
  ...
  contents.on("did-finish-load", () => {
    ...
    if (
      process.platform === "darwin" &&
      url.startsWith(runtimeConfig.urls.web)
    ) {
      void contents
  ...
  void (async () => {
    ...
    try {
      if (app.isPackaged) {
        ...
        // path, which is required to repair installs upgraded from a
        // pre-lift build.
