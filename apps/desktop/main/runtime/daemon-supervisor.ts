...
function findWindowsListeningTcpPid(port: number): number | null {
  try {
    ...
    for (const line of output.split(/\r?\n/)) {
      ...
      const [protocol, localAddress, , state, pidToken] = parts;
      if (!protocol.toUpperCase().startsWith("TCP")) {
        continue;
      }
...
export class RuntimeOrchestrator {
        ...
        record.manifest.command && record.manifest.args
          ...
          : record.manifest.launchStrategy === "launchd"
            ...
            : record.manifest.launchStrategy === "delegated"
              ? record.manifest.delegatedProcessMatch
                ? `delegated process match: ${record.manifest.delegatedProcessMatch}`
                : record.manifest.port !== null
                  ? `delegated port probe: ${record.manifest.port}`
                  : "delegated runtime"
              : null,
  ...
  private refreshDelegatedUnit(record: RuntimeUnitRecord): void {
    if (process.platform === "win32" && record.manifest.port !== null) {
      const previousPhase = record.phase;
      const previousPid = record.pid;
      const pid = findWindowsListeningTcpPid(record.manifest.port);
      ...
      if (pid !== null) {
        ...
        if (previousPhase !== record.phase || previousPid !== record.pid) {
          ...
          this.logStateChange(record, {
            ...
            message:
              `delegated runtime detected via tcp port ${record.manifest.port}: ` +
              `pid ${pid}`,
          });
        }
        return;
      ...
      if (previousPhase !== record.phase || previousPid !== record.pid) {
        ...
        this.logStateChange(record, {
          ...
          message:
            `delegated runtime ${record.manifest.id} is no longer listening ` +
            `on port ${record.manifest.port}`,
        });
      }
      return;
    }
