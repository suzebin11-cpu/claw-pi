...
async function getProductName() {
  ...
  return desktopPackage.build?.productName ?? desktopPackage.name;
}
...
async function stapleNotarizedAppBundles() {
  ...
  for (const entry of appBundleDirs) {
    const appPath = resolve(releaseRoot, entry.name, `${productName}.app`);
...
async function adhocSignAndRepackageUnsigned() {
  if (!isUnsigned) {
    return;
  }
  ...
  if (appBundleDirs.length === 0) {
    console.log("[dist:mac] no app bundles found for adhoc signing");
    return;
  }
  ...
  for (const entry of appBundleDirs) {
    ...
    const appId = desktopPackage.build?.appId ?? "app.clawpi.desktop";

    console.log(`[dist:mac] adhoc signing unsigned app bundle: ${appPath}`);
    await run("codesign", [
      "--sign",
      "-",
      "--force",
    ...
    console.log(
      `[dist:mac] re-signing outer bundle with stable designated requirement`,
    );
    await run("codesign", [
      "--sign",
      "-",
    ...
    console.log(`[dist:mac] verifying adhoc signature: ${appPath}`);
    await run("codesign", [
      "--verify",
      "--deep",
      "--verbose=2",
  ...
  const allNames = await readdir(releaseRoot);
  const zipNames = allNames.filter((name) => name.endsWith(".zip"));
  ...
  for (const zipName of zipNames) {
    ...
    let macDirName = "mac";
    if (zipName.includes("-arm64")) {
      macDirName = "mac-arm64";
    } else if (zipName.includes("-x64")) {
      macDirName = "mac-x64";
    }
    ...
    console.log(`[dist:mac] recreating zip with adhoc-signed app: ${zipPath}`);
    await rm(zipPath, rmWithRetriesOptions);
    await run("ditto", [
      "-c",
      "-k",
      "--sequesterRsrc",
    ...
    await rm(`${zipPath}.blockmap`, rmWithRetriesOptions);
  }

  const ymlPath = resolve(releaseRoot, "latest-mac.yml");
  try {
    let ymlContent = await readFile(ymlPath, "utf8");

    for (const zipName of zipNames) {
      const zipPath = resolve(releaseRoot, zipName);
      const sha512Base64 = await computeFileSha512(zipPath);
      const zipStat = await lstat(zipPath);
      const fileSize = zipStat.size;

      const escapedName = zipName.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
      ymlContent = ymlContent.replace(
        new RegExp(
          `(- url: ${escapedName}\\n\\s+sha512: )\\S+(\\n\\s+size: )\\d+`,
        ),
        `$1${sha512Base64}$2${fileSize}`,
      );
      ymlContent = ymlContent.replace(
        new RegExp(`(path: ${escapedName}\\nsha512: )\\S+`),
        `$1${sha512Base64}`,
      );
    }

    await writeFile(ymlPath, ymlContent);
    console.log("[dist:mac] updated latest-mac.yml with new checksums");
  } catch (err) {
    console.log(
      `[dist:mac] warning: could not update latest-mac.yml: ${err.message}`,
    );
  }
}
...
async function main() {
  ...
  await timedStep(
    "adhoc sign unsigned bundles",
    async () => adhocSignAndRepackageUnsigned(),
    timings,
  );
  await timedStep(
