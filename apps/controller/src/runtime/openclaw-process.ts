...
import { quarantineSubagentRunsForStartupSync } from "./openclaw-subagent-runs-quarantine.js";
...
export class OpenClawProcessManager {
  ...
  isStable(): boolean {
    ...
    if (this.controlledRestartTimer !== null) {
      return false;
    }
    const uptime = Date.now() - this.lastStartTime;
    return uptime >= STABLE_UPTIME_MS;
  }

  onStable(callback: () => void): () => void {
  ...
  start(): void {
    ...
    // Quarantine any persisted subagent runs before the gateway initializes
    // its registry. See openclaw-subagent-runs-quarantine.ts for full
    // rationale; the short version is that residual runs.json entries cause
    // the gateway to re-trigger the plugin loader 30-50 times during the
    // first 2-4 minutes of every (re)start.
    try {
    ...
    if (electronExec) {
      ...
      (this.env.openclawBin.endsWith(".cmd") ||
        this.env.openclawBin.endsWith(".bat"))
    ) {
      // Windows .cmd/.bat files must be invoked via cmd.exe to avoid EINVAL
      cmd = "cmd.exe";
      args = ["/c", this.env.openclawBin, "gateway", "run"];
    } else {
