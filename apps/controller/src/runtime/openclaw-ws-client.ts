...
/**
 * Minimal surface of {@link OpenClawProcessManager} consumed by the WS client.
 * Keeps the two modules decoupled for tests and avoids a hard cycle via the
 * container wiring.
 */
export interface OpenClawProcessLike {
  isAlive(): boolean;
}
...
// Two-stage tick watchdog thresholds.
//  - SOFT: log only. Frequently tripped by WeChat first-message cold start when
//    OpenClaw's event loop stalls for ~60s loading session/skill/agent state.
//  - HARD: force WS close and reconnect. Only fires after HARD and the process
//    is no longer observably alive, OR after a second consecutive HARD window
//    (i.e. event loop truly dead, not just slow).
const TICK_SOFT_TIMEOUT_MS = 60_000;
const TICK_HARD_TIMEOUT_MS = 180_000;
...
export class OpenClawWsClient {
  ...
  private processManager: OpenClawProcessLike | null;
  ...
  /**
   * Allow late wiring of the process manager (e.g. when the WS client is
   * constructed before the process manager in the container bootstrap order).
   */
  setProcessManager(pm: OpenClawProcessLike): void {
  ...
  private startTickWatch(): void {
    ...
    this.hardTimeoutTrippedAt = null;
    this.tickTimer = setInterval(
      () => {
        ...
        // Tick has recovered since the last hard trip — forget the latch so a
        // subsequent slow period gets its full grace again.
        if (gap <= TICK_SOFT_TIMEOUT_MS && this.hardTimeoutTrippedAt !== null) {
          logger.info({ gapMs: gap }, "openclaw_ws_tick_recovered");
          this.hardTimeoutTrippedAt = null;
          return;
        }

        if (gap <= TICK_SOFT_TIMEOUT_MS) {
          return;
        }

        // Soft threshold: event loop is slow but we don't close the socket.
        // The most common cause is WeChat first-message cold start (~60-120s
        // of synchronous session/skill loading). Dropping the WS here would
        // surface as a misleading "disconnected" in the UI.
        if (gap <= TICK_HARD_TIMEOUT_MS) {
          logger.warn({ gapMs: gap }, "openclaw_ws_tick_timeout_soft");
          return;
        }

        // Hard threshold. Only drop the WS if we have real evidence that the
        // OpenClaw process is gone (crashed / exited) OR we've already waited
        // through one full hard window and ticks are still missing. This
        // tolerates genuinely long stalls while still guaranteeing we recover
        // from a dead connection in bounded time.
        const processAlive = this.processManager?.isAlive() ?? true;
        const now = Date.now();
        if (!processAlive) {
          logger.warn(
            { gapMs: gap },
            "openclaw_ws_tick_timeout_hard_process_gone",
          );
          this.hardTimeoutTrippedAt = null;
        ...
        if (this.hardTimeoutTrippedAt === null) {
          this.hardTimeoutTrippedAt = now;
          logger.warn({ gapMs: gap }, "openclaw_ws_tick_timeout_hard_waiting");
          return;
        }

        if (now - this.hardTimeoutTrippedAt >= TICK_HARD_TIMEOUT_MS) {
          logger.warn(
            {
              gapMs: gap,
              hardWaitMs: now - this.hardTimeoutTrippedAt,
            },
            "openclaw_ws_tick_timeout_hard_final",
          );
          this.hardTimeoutTrippedAt = null;
          this.ws?.close(4000, "tick timeout");
        }
