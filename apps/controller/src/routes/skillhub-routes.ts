...
export function registerSkillhubRoutes(
  ...
  // GET /api/v1/skillhub/search — proxy search to mirror registry
  app.openapi(
    createRoute({
      method: "get",
      path: "/api/v1/skillhub/search",
      tags: ["SkillHub"],
      request: {
        query: z.object({
          q: z.string().default(""),
      ...
      responses: {
        200: {
          ...
          description: "Search results from mirror registry",
        },
      },
    }),
    async (c) => {
      const { q, limit, marker } = c.req.valid("query");
      const result = await container.skillhubService.catalog.searchRegistry(
        q,
        limit,
        marker,
      );
