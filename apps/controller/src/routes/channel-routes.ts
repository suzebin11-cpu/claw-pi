...
import {
  ...
  feishuDeviceBeginResponseSchema,
  feishuDevicePollRequestSchema,
  feishuDevicePollResponseSchema,
...
export function registerChannelRoutes(
  ...
  app.openapi(
    createRoute({
      method: "post",
      path: "/api/v1/channels/feishu/device-begin",
      tags: ["Channels"],
      responses: {
        200: {
          content: {
            "application/json": { schema: feishuDeviceBeginResponseSchema },
          },
          description: "Feishu device flow QR code data",
        },
        502: {
          content: { "application/json": { schema: errorSchema } },
          description: "Feishu device flow unavailable",
        },
      },
    }),
    async (c) => {
      try {
        return c.json(await container.channelService.feishuDeviceBegin(), 200);
      } catch (error) {
        return c.json(
          {
            message:
              error instanceof Error
                ? error.message
                : "Failed to start Feishu device flow",
          },
          502,
        );
      }
  ...
  app.openapi(
    createRoute({
      method: "post",
      path: "/api/v1/channels/feishu/device-poll",
      tags: ["Channels"],
      request: {
        body: {
          required: true,
          content: {
            "application/json": { schema: feishuDevicePollRequestSchema },
          },
        },
      },
      responses: {
        200: {
          content: {
            "application/json": { schema: feishuDevicePollResponseSchema },
          },
          description: "Feishu device flow poll result",
        },
        502: {
          content: { "application/json": { schema: errorSchema } },
          description: "Feishu device flow poll failed",
        },
      },
    }),
    async (c) => {
      try {
        ...
        return c.json(
          await container.channelService.feishuDevicePoll(deviceCode),
          200,
        );
      } catch (error) {
        return c.json(
          {
            message:
              error instanceof Error
                ? error.message
                : "Feishu device flow poll failed",
          },
          502,
        );
      }
  ...
  const channelLiveStatusEntrySchema = z.object({
    ...
    configured: z.boolean(),
    lastError: z.string().nullable(),
    // Present when the entry is served from a sticky snapshot while the
    // gateway is transiently unreachable. The UI shows a soft "syncing…"
    // hint instead of downgrading the status pill.
