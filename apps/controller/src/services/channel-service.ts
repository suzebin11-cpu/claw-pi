...
import { quarantineSubagentRunsForStartup } from "../runtime/openclaw-subagent-runs-quarantine.js";
...
function activateWeChatStoredAccount(
  env: ControllerEnv,
  accountId: string,
  data: WechatStoredAccount,
): void {
  ...
  logger.info(
    ...
    "wechat_account_store_activated",
  );
}
...
async function pollWechatQrStatus(
  ...
  qrcode: string,
  headers: Record<string, string>,
  timeoutMs = WECHAT_QR_POLL_TIMEOUT_MS,
...
export class ChannelService {
  ...
  async wechatQrWait(sessionKey: string) {
    ...
    const deadline = Date.now() + WECHAT_QR_POLL_TIMEOUT_MS;
    const headers = buildWechatQrHeaders(this.env, {
    ...
    while (Date.now() < deadline) {
      ...
      if (
        ...
        activateWeChatStoredAccount(this.env, normalizedAccountId, {
  ...
  async feishuDeviceBegin() {
    const initRes = await proxyFetch(
    ...
    if (!initData.supported_auth_methods?.includes("client_secret")) {
      throw new Error("Feishu environment does not support device flow");
    }

    const beginRes = await proxyFetch(
      "https://accounts.feishu.cn/oauth/v1/app/registration",
      {
        method: "POST",
        headers: {
    ...
    if (!beginData.verification_uri_complete || !beginData.device_code) {
      throw new Error("Failed to start Feishu device flow");
    }

    return {
      qrUrl: beginData.verification_uri_complete,
  ...
  async feishuDevicePoll(deviceCode: string) {
    ...
    if (pollData.error === "authorization_pending") {
      return { done: false as const, status: "waiting" };
    }

    if (pollData.error === "expired_token") {
      return { done: false as const, status: "expired" };
    }

    return { done: false as const, status: pollData.error ?? "unknown" };
  }
  ...
  async disconnectChannel(channelId: string) {
    ...
    if (removed) {
      // syncAll triggers the authoritative index writer, which updates active
      // account IDs. Credential and sync files are preserved so disconnect
      // stays a pure "unbind", not a destructive logout.
      await this.syncService.syncAll();
  ...
  private async restartOpenClawForWhatsappLifecycle(reason: string) {
    ...
    if (this.env.manageOpenclawProcess) {
      // OpenClawProcessManager.start() runs the quarantine hook for us.
      ...
      // launchd-managed sidecars bypass OpenClawProcessManager.start(), so we
      // have to run the quarantine ourselves before kicking the service.
      // Failures inside the helper are already logged; never let them block
      // the actual restart command.
      await quarantineSubagentRunsForStartup({
        ...
        logger.warn(
          ...
          "subagent_runs_quarantine_unexpected_failure",
        );
      });
