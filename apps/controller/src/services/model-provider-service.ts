...
function isEquivalentKnownModelId(currentId: string, knownId: string): boolean {
  if (currentId === knownId) {
    return true;
  }

  const currentIsLink = currentId.startsWith("link/");
  const knownIsLink = knownId.startsWith("link/");
  if (currentIsLink || knownIsLink) {
    return normalizeLinkModelId(currentId) === normalizeLinkModelId(knownId);
  }
...
export class ModelProviderService {
  ...
  private async getDefaultModelValidity(): Promise<DefaultModelValidity> {
    ...
    return knownModels.some((model) =>
      isEquivalentKnownModelId(currentId, model.id),
    )
      ? "valid"
      : "invalid";
  }
