...
export type InstallFailedCallback = (
  slug: string,
  source: SkillSource,
...
export class InstallQueue {
  ...
  private readonly onFailed: InstallFailedCallback | null;
  ...
  constructor(opts: {
    ...
    onFailed?: InstallFailedCallback;
    log?: LogFn;
      ...
      (err: unknown) => {
        ...
        if (pauseMs !== null) {
          ...
          if (item.retries >= this.maxRetries) {
            ...
            this.active.delete(item.slug);
            this.completed.push(item);
            this.onFailed?.(item.slug, item.source, code);
            this.scheduleCleanup(item);
            this.drain();
            return;
          }
