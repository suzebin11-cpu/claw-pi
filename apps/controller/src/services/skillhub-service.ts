...
export class SkillhubService {
  ...
  static async create(
    ...
    const catalogManager = new CatalogManager(env.skillhubCacheDir, {
      ...
      skillDb,
      log,
      clawHubRegistry: env.clawHubRegistry,
      clawHubSearchApi: env.clawHubSearchApi,
    });
    ...
    const installQueue = new InstallQueue({
      ...
      onFailed: (slug, source, errorCode) => {
        if (errorCode === "skill_not_found") {
          skillDb.recordUninstall(slug, source);
          log("info", `Marked unavailable skill as uninstalled: ${slug}`);
        }
      },
      log,
    });
  ...
  private initialize(): void {
    ...
    if (this.env.staticSkillsDir && existsSync(this.env.staticSkillsDir)) {
      ...
      for (const entry of failed) {
        this.log(
          "warn",
          `static skill copy failed slug=${entry.slug}: ${entry.error}`,
        );
      }
    }
