...
const controllerBootTimestamp = Date.now();
...
export async function createContainer(): Promise<ControllerContainer> {
  ...
  await configStore.reconcileSingleWechatChannel();
  ...
  return {
    env,
    bootTimestamp: controllerBootTimestamp,
    gatewayClient,
    runtimeHealth,
    openclawProcess,
    ...
    imageGenerationService: new ImageGenerationService(configStore, env),
    analyticsService,
