...
describe("SkillhubService", () => {
  ...
  it("bootstrap() continues when static skill copy reports failures", async () => {
    const env = createEnv(rootDir);
    const staticDir = path.join(rootDir, "static-skills");
    mkdirSync(staticDir, { recursive: true });
    const envWithStatic = { ...env, staticSkillsDir: staticDir };
    ...
    mocks.mockCopyStaticSkills.mockReturnValueOnce({
      copied: [],
      skipped: [],
      failed: [{ slug: "libtv-video", error: "EIO: Access is denied" }],
    });
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "[skillhub] static skill copy failed slug=libtv-video: EIO: Access is denied",
    );
  });

  it("start() still enqueues curated installs when static skill copy reports failures", async () => {
    const env = createEnv(rootDir);
    const staticDir = path.join(rootDir, "static-skills");
    mkdirSync(staticDir, { recursive: true });
    const envWithStatic = { ...env, staticSkillsDir: staticDir };
    ...
    const service = await SkillhubService.create(envWithStatic);
    const catalog = mocks.catalogManagerInstances[0];
    catalog.getCuratedSlugsToEnqueue.mockReturnValue(["alpha"]);
