...
describe("ask workbench routing", () => {
  ...
  it("routes short direct-open continuations to write_agent with recent artifact context", () => {
    expect(classify("直接打开", ["已生成网页文件：有色金属财经简报.html"])).toBe(
      "write_agent",
    );
  });

  it("keeps read-only webpage analysis as read_only_agent", () => {
    expect(classify("帮我总结这个网页内容")).toBe("read_only_agent");
  });

  it("keeps ordinary chat and image generation routes unchanged", () => {
