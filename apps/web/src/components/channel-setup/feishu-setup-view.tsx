...
import {
  ...
  postApiV1ChannelsFeishuDeviceBegin,
  postApiV1ChannelsFeishuDevicePoll,
    ...
    <div className={variant === "modal" ? "" : ""}>
      ...
      <div className="flex gap-2 mb-5">
        <button
          ...
          className={`flex items-center gap-1.5 px-3.5 py-2 text-[12px] font-medium rounded-lg transition-all cursor-pointer ${
            mode === "quick"
              ? "bg-[#3370FF]/10 text-[#3370FF] border border-[#3370FF]/30"
              : "bg-surface-1 text-text-muted border border-border hover:border-border-hover"
          }`}
        >
          <QrCode size={13} />
          {t("feishuSetup.quickSetupTab")}
        </button>
        <button
          type="button"
        ...
        >
          <Settings2 size={13} />
          {t("feishuSetup.manualSetupTab")}
        </button>
      </div>

      {mode === "quick" ? (
        <FeishuDeviceFlowView onConnected={onConnected} disabled={disabled} />
      ) : (
        <FeishuManualSetupView onConnected={onConnected} disabled={disabled} />
      )}
    </div>
  );
}
...
type DeviceFlowState =
  | { phase: "idle" }
  | { phase: "loading" }
  | { phase: "scanning"; qrUrl: string; deviceCode: string; expireAt: number }
  | { phase: "connecting" }
  | { phase: "expired" }
  | { phase: "error"; message: string };

function FeishuDeviceFlowView({
  onConnected,
  disabled,
}: {
  onConnected: () => void;
  ...
  const startDeviceFlow = async () => {
    ...
    try {
      const { data, error } = await postApiV1ChannelsFeishuDeviceBegin();

      if (error || !data) {
        setState({
          phase: "error",
          message: t("feishuSetup.deviceFlowStartFailed"),
        });
        return;
      }

      const expireAt = Date.now() + data.expireIn * 1000;
      setState({
        phase: "scanning",
        qrUrl: data.qrUrl,
      ...
      startPolling(data.deviceCode, data.interval, expireAt);
    } catch {
      setState({
        phase: "error",
        message: t("feishuSetup.deviceFlowStartFailed"),
  ...
  const startPolling = (
    ...
    const poll = async () => {
      ...
      try {
        const { data, error } = await postApiV1ChannelsFeishuDevicePoll({
          body: { deviceCode },
        });
        ...
        if (data.done && data.appId && data.appSecret) {
          setState({ phase: "connecting" });
          await connectWithCredentials(data.appId, data.appSecret);
          return;
        }
  ...
  const connectWithCredentials = async (appId: string, appSecret: string) => {
    try {
      ...
      if (error) {
        setState({
          phase: "error",
          message: t("feishuSetup.connectFailed"),
        });
        return;
      }
      ...
      track("channel_ready", {
        channel: "feishu",
        channel_type: "feishu_device_flow",
      });
    ...
    <div className="p-5 rounded-xl border bg-surface-1 border-border">
      <div className="flex gap-3 items-start mb-4">
        ...
        <div>
          <h3 className="text-[14px] font-semibold text-text-primary">
            {t("feishuSetup.deviceFlowTitle")}
          </h3>
          <p className="text-[12px] text-text-muted mt-1 leading-relaxed">
            {t("feishuSetup.deviceFlowDesc")}
          </p>
        </div>
      </div>
      ...
      {state.phase === "loading" && (
        <div className="flex items-center gap-2 text-[13px] text-text-muted">
          ...
          {t("feishuSetup.generatingQr")}
        </div>
      )}

      {state.phase === "scanning" && (
        <div className="space-y-4">
          <div className="flex justify-center p-6 bg-surface-1 rounded-lg border border-border">
            <QRCodeSVG value={state.qrUrl} size={208} />
          </div>
          <div className="flex items-center gap-2 text-[12px] text-text-muted">
            <Loader2 size={12} className="animate-spin" />
            {t("feishuSetup.waitingScan")}
          </div>
        </div>
      )}

      {state.phase === "connecting" && (
        <div className="flex items-center gap-2 text-[13px] text-[#3370FF]">
          <Loader2 size={14} className="animate-spin" />
          {t("feishuSetup.autoConnecting")}
        </div>
      )}
      ...
      {state.phase === "expired" && (
        <div className="space-y-3">
          <p className="text-[12px] text-text-muted">
            {t("feishuSetup.qrExpired")}
          </p>
          <button
            type="button"
            onClick={startDeviceFlow}
            disabled={disabled}
            className="flex gap-1.5 items-center px-4 py-2 text-[12px] font-medium text-white rounded-lg bg-[#3370FF] hover:bg-[#2860E6] transition-all disabled:opacity-60 cursor-pointer"
          >
            <QrCode size={13} />
            {t("feishuSetup.retryScan")}
          </button>
        </div>
      )}
...
// ─── Manual Setup (original 3-step flow) ────────────────────

function FeishuManualSetupView({
  onConnected,
  disabled,
}: {
  onConnected: () => void;
