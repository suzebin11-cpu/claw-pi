...
function SkillCard({
  ...
  const tagStyle = getSkillTagStyle(skill.tags);
  ...
  const cardContent = (
    <>
      ...
      {/* Description */}
      <p className="text-[12.5px] text-text-secondary leading-[1.55] line-clamp-2 mb-3">
        {skill.description}
      </p>
...
export function SkillsPage() {
  ...
  const exploreSkills = exploreQuery.skills;
  ...
  const filteredYoursSkills = useMemo(() => {
    ...
    return list;
  }, [yourSkillsList, activeTag, debouncedQuery]);

  const filteredSkills =
    topTab === "explore" ? exploreSkills : filteredYoursSkills;
  ...
  }, [topTab, exploreQuery, filteredSkills.length]);
