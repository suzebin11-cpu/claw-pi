...
describe("LaunchdManager", () => {
  ...
  describe("getServiceStatus", () => {
    it("parses running service with PID", async () => {
      ...
      const status = await mgr.getServiceStatus("app.clawpi.controller");
    ...
    it("parses stopped service", async () => {
      setupExecFile({
        "launchctl print": {
          stdout: ["app.clawpi.controller = {", "\tstate = waiting", "}"].join(
            "\n",
          ),
        },
      });
  ...
  describe("installService", () => {
    it("writes plist and bootstraps when not registered", async () => {
      ...
      await mgr.installService("app.clawpi.controller", "<plist>test</plist>");
      ...
      const plistPath = (fs.writeFile as ReturnType<typeof vi.fn>).mock
        .calls[0]?.[0];
      expect(typeof plistPath).toBe("string");
      expect(toPosix(plistPath as string)).toBe(
        "/tmp/test/app.clawpi.controller.plist",
      );
    });
  ...
  describe("isServiceInstalled", () => {
    it("returns true when plist exists and service is registered", async () => {
      ...
      const mgr = new LaunchdManager({ plistDir: "/tmp/test" });
      const result = await mgr.isServiceInstalled("app.clawpi.controller");
  ...
  describe("uninstallService", () => {
    it("bootouts and deletes plist file", async () => {
      ...
      await mgr.uninstallService("app.clawpi.controller");

      expect(fs.unlink).toHaveBeenCalledWith(expect.any(String));
      const plistPath = (fs.unlink as ReturnType<typeof vi.fn>).mock
        .calls[0]?.[0];
      expect(typeof plistPath).toBe("string");
      expect(toPosix(plistPath as string)).toBe(
        "/tmp/test/app.clawpi.controller.plist",
  ...
  describe("stopServiceGracefully", () => {
    it("stops service and returns when status becomes stopped", async () => {
      ...
      await mgr.stopServiceGracefully("app.clawpi.controller", 5000);
    ...
    it("sends SIGKILL after timeout", async () => {
      ...
      await mgr.stopServiceGracefully("app.clawpi.controller", 500);
    ...
    it("returns immediately when stopService fails (already stopped)", async () => {
      ...
      // Should not throw
      await mgr.stopServiceGracefully("app.clawpi.controller");
    });
  });
  ...
  describe("restartService", () => {
    it("calls launchctl kickstart -k", async () => {
      ...
      await mgr.restartService("app.clawpi.controller");
  ...
  describe("rebootstrapFromPlist", () => {
    it("bootstraps from existing plist file", async () => {
      ...
      await mgr.rebootstrapFromPlist("app.clawpi.controller");
    ...
    it("throws when bootstrap fails", async () => {
      ...
      await expect(
        mgr.rebootstrapFromPlist("app.clawpi.controller"),
  ...
  describe("hasPlistFile", () => {
    it("returns true when plist file exists", async () => {
      ...
      const result = await mgr.hasPlistFile("app.clawpi.controller");
      expect(result).toBe(true);
    });
