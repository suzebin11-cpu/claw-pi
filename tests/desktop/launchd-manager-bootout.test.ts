...
describe("LaunchdManager.bootoutService", () => {
  ...
  it("resolves successfully when launchctl bootout succeeds", async () => {
    ...
    await expect(
      manager.bootoutService("app.clawpi.test"),
    ...
    mockExecFile.mockImplementation(
      (_cmd: string, _args: string[], callback: (err: Error) => void) => {
        const error = new Error(
          "Command failed: launchctl bootout gui/501/app.clawpi.test\n" +
  ...
  it("re-throws unexpected errors", async () => {
    ...
    await expect(manager.bootoutService("app.clawpi.test")).rejects.toThrow(
      "Permission denied",
    );
  });
});
