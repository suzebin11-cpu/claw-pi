...
// ---------------------------------------------------------------------------
// Local update server that mimics the remote update endpoint
// ---------------------------------------------------------------------------
...
describe("Update server integration", () => {
  ...
  it("latest-mac.yml contains all fields required by electron-updater", async () => {
    ...
    expect(body).toMatch(/url: nexu-.*\.zip/);
  ...
  it("download artifact URL serves content", async () => {
    const response = await fetch(
      `${serverBaseUrl}/stable/arm64/nexu-${LATEST_VERSION}-arm64.zip`,
    );
    expect(response.ok).toBe(true);
  });
  ...
  // -----------------------------------------------------------------------
  it("github source falls back to R2 feed URL", () => {
    const url = resolveUpdateFeedUrlForTests({
      source: "github",
      channel: "stable",
      feedUrl: null,
  ...
  // -----------------------------------------------------------------------
  it("CLAWPI_UPDATE_FEED_URL env overrides everything", () => {
    const originalEnv = process.env.CLAWPI_UPDATE_FEED_URL;
    try {
      process.env.CLAWPI_UPDATE_FEED_URL = `${serverBaseUrl}/env-override`;
      ...
      if (originalEnv === undefined) {
        ...
        process.env.CLAWPI_UPDATE_FEED_URL = originalEnv;
      }
    }
  });
});
