...
### Service labels
- Production: `app.clawpi.controller`, `app.clawpi.openclaw`
- Dev mode: `app.clawpi.controller.dev`, `app.clawpi.openclaw.dev`
...
The controller and clawhub CLI are each bundled into single-file ESM bundles via esbuild (`apps/controller/scripts/build-bundle.mjs`). The build script (`tsc --noEmit && node scripts/build-bundle.mjs && node scripts/bundle-runtime-plugins.mjs`) produces:

- `dist/index.js` — controller server bundle (all deps inlined)
- `dist/clawhub.js` — clawhub CLI bundle (invoked as a subprocess for skill install/uninstall)
- `dist/*.js.map` — external sourcemaps

The sidecar preparation script (`apps/desktop/scripts/prepare-controller-sidecar.mjs`) copies `dist/`, `static/`, `plugins/`, and a minimal `package.json` into the sidecar directory. **No `node_modules/` is included** — all controller dependencies are bundled into the two ESM files. This eliminates thousands of small files that trigger slow Windows Defender real-time scans on first launch.

Runtime plugins (dingtalk, wecom, qqbot) are separately packaged by `bundle-runtime-plugins.mjs` into `plugins/` with their own dependencies.

**Rules:**

- **Native Node.js addons** (e.g. `better-sqlite3`) must live in the controller, NOT in the Electron main process. Electron's built-in Node.js uses a different ABI version (`NODE_MODULE_VERSION`) from system Node.js, which causes "compiled against a different Node.js version" errors. The controller runs as a regular Node.js process (`ELECTRON_RUN_AS_NODE=1`), so native addons work without `electron-rebuild`. Note: native addons cannot be bundled by esbuild and would need to remain as external files.
- **CJS dependencies** are supported via a `createRequire` shim injected by the esbuild banner. If a new CJS dependency causes issues, check the bundle output for `Dynamic require of "X" is not supported` errors.
...
- External controller sidecar: `~/.nexu/runtime/controller-sidecar/` (APFS-cloned esbuild bundles, no node_modules)
- External openclaw sidecar: `~/.nexu/runtime/openclaw-sidecar/` (extracted from .app payload)
- Launchd plist directory: `~/Library/LaunchAgents/` (`app.clawpi.controller.plist`, `app.clawpi.openclaw.plist`)
