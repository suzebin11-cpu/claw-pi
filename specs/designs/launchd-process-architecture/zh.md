...
|------|---------------|------|-----------|
| **Desktop** | `app.clawpi.desktop` | GUI Shell + 内置 Web Server | No (用户手动启动) |
| **Controller** | `app.clawpi.controller` | API Server, 配置管理, 状态存储 | Yes (崩溃自动重启) |
| **OpenClaw** | `app.clawpi.openclaw` | Bot Runtime, Channel 连接 | Yes (崩溃自动重启) |

> **与原生 OpenClaw 隔离**: Claw-Pi Desktop 使用 `app.clawpi.*` 命名空间作为 launchd label，与 OpenClaw 原生 `openclaw install` 使用的 `io.openclaw.*` 完全独立。状态目录也分离（`~/.claw-pi/` vs `~/.openclaw/`）。用户可以同时运行 Claw-Pi Desktop 和独立的 OpenClaw 实例，互不冲突。
