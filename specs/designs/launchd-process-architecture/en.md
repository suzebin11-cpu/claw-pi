...
│                                                                  │
│  ┌────────────────────┐                                         │
│  │ app.clawpi.desktop   │  Electron GUI Shell                     │
...
│            │                                                     │
│  ┌─────────▼──────────┐  ┌───────────────────┐                  │
│  │ app.clawpi.controller│  │ app.clawpi.openclaw │                  │
...
|---------|---------------|------------------|-----------|
| **Desktop** | `app.clawpi.desktop` | GUI Shell + Embedded Web Server | No (user-launched) |
| **Controller** | `app.clawpi.controller` | API Server, config management, state storage | Yes (auto-restart on crash) |
| **OpenClaw** | `app.clawpi.openclaw` | Bot Runtime, channel connections | Yes (auto-restart on crash) |

> **Isolation from Native OpenClaw**: Claw-Pi Desktop uses the `app.clawpi.*` namespace for launchd labels, completely independent from OpenClaw's native `openclaw install` which uses `io.openclaw.*`. State directories are also separate (`~/.claw-pi/` vs `~/.openclaw/`). Users can run Claw-Pi Desktop and a standalone OpenClaw instance simultaneously without conflicts.
...
<dict>
    <key>Label</key>
    <string>app.clawpi.controller</string>
...
<dict>
    <key>Label</key>
    <string>app.clawpi.openclaw</string>
...
|----------|-------------|------------|
| `Label` | `app.clawpi.controller.dev` | `app.clawpi.controller` |
...
export async function bootstrapWithLaunchd(env: DesktopEnv): Promise<void> {
  ...
  const labels = {
    controller: env.isDev ? "app.clawpi.controller.dev" : "app.clawpi.controller",
    openclaw: env.isDev ? "app.clawpi.openclaw.dev" : "app.clawpi.openclaw",
  };
...
const SERVICE_LABELS = {
  controller: app.isPackaged ? "app.clawpi.controller" : "app.clawpi.controller.dev",
  openclaw: app.isPackaged ? "app.clawpi.openclaw" : "app.clawpi.openclaw.dev",
};
...
cleanup() {
  ...
  # launchctl kill SIGTERM gui/$(id -u)/app.clawpi.controller.dev 2>/dev/null || true
  # launchctl kill SIGTERM gui/$(id -u)/app.clawpi.openclaw.dev 2>/dev/null || true
}
...
if ! launchctl bootstrap gui/$UID_VAL "$PLIST_DIR/app.clawpi.controller.dev.plist" 2>&1; then
  echo "Note: Controller service may already be bootstrapped"
fi
if ! launchctl bootstrap gui/$UID_VAL "$PLIST_DIR/app.clawpi.openclaw.dev.plist" 2>&1; then
  echo "Note: OpenClaw service may already be bootstrapped"
fi

# Kickstart
launchctl kickstart -k gui/$UID_VAL/app.clawpi.controller.dev
launchctl kickstart -k gui/$UID_VAL/app.clawpi.openclaw.dev
...
└── .tmp/
    ├── launchd/
    │   ├── app.clawpi.controller.dev.plist
    │   └── app.clawpi.openclaw.dev.plist
    ├── logs/
    │   ├── controller.log
    │   ├── controller.err
    │   ├── openclaw.log
...
launchctl print gui/$(id -u)/app.clawpi.controller

# Start service
launchctl kickstart gui/$(id -u)/app.clawpi.controller

# Restart service (-k = kill first)
launchctl kickstart -k gui/$(id -u)/app.clawpi.controller

# Stop service
launchctl kill SIGTERM gui/$(id -u)/app.clawpi.controller

# Install plist
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/app.clawpi.controller.plist

# Uninstall service
launchctl bootout gui/$(id -u)/app.clawpi.controller
```
...
│       └── workspace-templates/
├── LaunchAgents/
│   ├── app.clawpi.controller.plist
│   └── app.clawpi.openclaw.plist
└── Logs/Nexu/
    ├── controller.log
    ├── controller.err
    ├── openclaw.log
