...
const svgSource = readFileSync(
  join(__dirname, "..", "..", "..", "apps", "desktop", "build", "icon.svg"),
);
...
async function main() {
  console.log("Generating docs apple-touch-icon.png (180x180)...");
  writeFileSync(join(__dirname, "apple-touch-icon.png"), await renderPng(180));

  console.log("Generating docs favicon.ico...");
  const icoSizes = [16, 24, 32, 48];
  const icoBuffers = await Promise.all(icoSizes.map(renderPng));
  writeFileSync(join(__dirname, "favicon.ico"), createIcoBuffer(icoBuffers, icoSizes));
